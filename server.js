...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .tasks-container { padding: 0 20px; }
    .section-title {
      font-size: 18px;
      margin-bottom: 15px;
      opacity: 0.8;
    ...
    .drag-hint { font-size: 12px; opacity: 0.6; font-weight: normal; }
    .task-card {
    ...
    .task-card:active { cursor: grabbing; transform: scale(1.02); box-shadow: 0 10px 30px rgba(0,0,0,0.3); }
    .task-card.dragging { opacity: 0.5; border: 2px dashed #667eea; background: rgba(102, 126, 234, 0.1); }
    .task-card.completed {
      opacity: 0.6;
    ...
    .due-date { font-size: 12px; opacity: 0.8; display: flex; align-items: center; gap: 4px; }
    ...
    .check-btn:hover { background: #4ECDC4; }
    ...
    .form-label { display: block; margin-bottom: 8px; font-size: 14px; opacity: 0.8; }
...
<body>
  ...
  <script>
    ...
    function requestNotificationPermission() {
      ...
      Notification.requestPermission().then(permission => {
        if (permission === 'granted') console.log('Permission accordée');
      });
    }
    
    function showNotification(title, body) {
      if (Notification.permission !== 'granted') return;
      const options = { body: body, icon: 'https://cdn-icons-png.flaticon.com/512/2098/2098402.png' };
  ...
  function renderTask(todo) {
    ...
    const dateStr = dueDate ? dueDate.toLocaleString('fr-FR', {
      day: 'numeric', month: 'short', hour: '2-digit', minute:'2-digit'
    }) : '';
    
    const draggable = !todo.completed ? 'draggable="true"' : '';
    const dragHandle = !todo.completed ? '<div class="drag-handle">☰</div>' : '';
        ...
        <div class="task-content">
          <div class="task-header-row">
            ...
            <div class="task-actions">
              <form method="POST" action="/toggle/${todo.id}" style="margin:0;">
                <button type="submit" class="icon-btn check-btn">
                  ${todo.completed ? '↩️' : '✓'}
                </button>
              </form>
          ...
          <div class="task-meta">
            ...
            ${dateStr ? `<span class="due-date ${isOverdue ? 'overdue' : ''}">📅 ${isOverdue ? '⚠️ ' : ''}${dateStr}</span>` : ''}
          </div>
        </div>
      </div>
    `;
...
app.post('/reorder', (req, res) => {
  ...
  const reordered = orderedIds.map(id => todoMap[id]).filter(Boolean);
  const remaining = todos.filter(t => !orderedIds.includes(t.id));
  saveTodos([...reordered, ...remaining]);
